...
sys.path.append(r"C:\Users\tlero\Documents\01_Cours Centrale Nantes\01_OCEAN\08_Projet\self-righting-boat-main\packages\hydrostatic\src\hydrostatic")

from hydrostatic_2d import join_polygons
...
def generate_arch_boat_ellipse():
    ...
    e = 0.25 # Épaisseur de l'arceau
    draft_offset = 1  # Décalage vers le bas des Y de 1 mètre

    # Dimensions de l'ellipse
    ellipse_a_sup = width / 2  # Demi-grand axe
    ellipse_b_sup = 1.5*height  # Demi-petit axe
    ellipse_a_inf = ellipse_a_sup - e
    ellipse_b_inf = ellipse_b_sup - e

    # Rectangle extérieur : base du bateau
    rect_x = np.linspace(-width / 2, width / 2, 10)
    rect_bottom = [(x, -height / 2 - draft_offset) for x in rect_x]
    rect_right = [(width / 2, y - draft_offset) for y in np.linspace(-height / 2, height / 2, 5)]
    rect_left = [(-width / 2, y - draft_offset) for y in np.linspace(height / 2, -height / 2, 5)]

    # Éllypse supérieure : centrée au-dessus du rectangle
    theta = np.linspace(0, np.pi, 10)
    ellipse_sup = [(ellipse_a_sup * np.cos(t), height / 2 + ellipse_b_sup * np.sin(t) - draft_offset) for t in theta]

    # Éllypse intérieure : centrée au même endroit
    ellipse_inf = [(ellipse_a_inf * np.cos(t), height / 2 + ellipse_b_inf * np.sin(t) - draft_offset) for t in reversed(theta)]

    # Assemblage des contours
...
def generate_arch_boat_squared():
    ...
    width = 4  # Largeur du rectangle
    height = 2  # Hauteur du rectangle
    e = 0.4  # Épaisseur de l'arceau
    draft_offset = 1  # Décalage vers le bas des Y de 1 mètre

    # Rectangle extérieur : base du bateau
    rect_x = np.linspace(-width / 2, width / 2, 10)
    rect_bottom = [(x, -height / 2 - draft_offset) for x in rect_x]
    rect_right = [(width / 2, y - draft_offset) for y in np.linspace(-height / 2, height / 2 + e, 5)]
    rect_left = [(-width / 2, y - draft_offset) for y in np.linspace(height / 2 + e, -height / 2, 5)]
    rect_top = [(x, height / 2 + e - draft_offset) for x in rect_x]
    
    # Rectangle intérieur : arceau intérieur
    inner_right = [(width / 2 - e, y - draft_offset) for y in np.linspace(height / 2 + e, -height / 2, 5)]
    inner_left = [(-width / 2 + e, y - draft_offset) for y in np.linspace(-height / 2, height / 2 + e, 5)]
    inner_bottom = [(x, -height / 2 + e - draft_offset) for x in np.linspace(-width / 2 + e, width / 2 - e, 10)]
    inner_top = [(x, height / 2 - draft_offset) for x in np.linspace(width / 2 - e, -width / 2 + e, 10)]
...
def generate_square_boat() -> tuple[list[tuple[float, float]], tuple[float, float]]:
    ...
    draft_offset = -0.5  # Lowers all Y coordinates by 1 meter
...
def generate_rectangle_boat() -> tuple[list[tuple[float, float]], tuple[float, float]]:
    ...
    width = 4   # Largeur du rectangle
    height = 2  # Hauteur du rectangle
    draft_offset = 1  # Décalage vertical vers le bas

    # Définition des sommets du rectangle (dans le sens antihoraire)
    rect_points = [
        (-width / 2, -height / 2 - draft_offset),  # Bas gauche
        (width / 2, -height / 2 - draft_offset),   # Bas droit
        (width / 2, height / 2 - draft_offset),    # Haut droit
        (-width / 2, height / 2 - draft_offset),   # Haut gauche
        (-width / 2, -height / 2 - draft_offset)   # Retour au point de départ pour fermer le contour
    ]

    # Centre de gravité : au centre du rectangle
    center_of_gravity = (0, -draft_offset)
...
if __name__ == "__main__":
    for method in [generate_arch_boat_inner_outer, generate_arch_boat, generate_arch_boat_squared, generate_arch_boat_ellipse]:
